...
The [BenchMARL](https://github.com/facebookresearch/BenchMARL) library provides state-of-the-art
...
For ease of use, this tutorial will follow the general structure of the already available [Multi-Agent Reinforcement Learning (PPO) with TorchRL Tutorial](multiagent_ppo.html).

In this tutorial, we will use the *simple_tag* environment from the
[MADDPG paper](https://arxiv.org/abs/1706.02275). This environment is part
of a set called [MultiAgentParticleEnvironments (MPE)](https://github.com/openai/multiagent-particle-envs)
...
- [PettingZoo](https://pettingzoo.farama.org/), in the traditional CPU version of the environment;
- [VMAS](https://github.com/proroklab/VectorizedMultiAgentSimulator), which provides a vectorized implementation in PyTorch,
able to simulate multiple environments on a GPU to speed up computation.

![Simple tag](https://github.com/matteobettini/vmas-media/blob/main/media/scenarios/simple_tag.gif?raw=true)
...
- How we can use [`TensorDict`](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict) to carry multi-agent multi-group data;
...
![Off-policy learning](https://pytorch.s3.amazonaws.com/torchrl/github-artifacts/img/off-policy-vmas-loop-min.png)

Off-policy learning

This approach has been extended to multi-agent learning in [Multi-Agent Actor-Critic for Mixed Cooperative-Competitive Environments](https://arxiv.org/abs/1706.02275),
...
For more info on grouping, see [`MarlGroupMapType`](../reference/generated/torchrl.envs.MarlGroupMapType.html#torchrl.envs.MarlGroupMapType).
...
functionality is already provided in the PettingZoo and VMAS simulators but the TorchRL [`StepCounter`](../reference/generated/torchrl.envs.transforms.StepCounter.html#torchrl.envs.transforms.StepCounter)
...
**Second**: wrap the `TensorDictModule` in a [`ProbabilisticActor`](../reference/generated/torchrl.modules.tensordict_module.ProbabilisticActor.html#torchrl.modules.tensordict_module.ProbabilisticActor)

We now need to build the TanhDelta distribution.
We instruct the [`ProbabilisticActor`](../reference/generated/torchrl.modules.tensordict_module.ProbabilisticActor.html#torchrl.modules.tensordict_module.ProbabilisticActor)
class to build a [`TanhDelta`](../reference/generated/torchrl.modules.TanhDelta.html#torchrl.modules.TanhDelta) out of the policy action
parameters. We also provide the minimum and maximum values of this
distribution, which we gather from the environment specs.

The name of the `in_keys` (and hence the name of the `out_keys` from
the `TensorDictModule` above) has to end with the
[`TanhDelta`](../reference/generated/torchrl.modules.TanhDelta.html#torchrl.modules.TanhDelta) distribution constructor keyword arguments (param).
...
In this case we use a [`AdditiveGaussianModule`](../reference/generated/torchrl.modules.AdditiveGaussianModule.html#torchrl.modules.AdditiveGaussianModule), which adds gaussian noise to our action
...
This buffer uses [`LazyMemmapStorage`](../reference/generated/torchrl.data.replay_buffers.LazyMemmapStorage.html#torchrl.data.replay_buffers.LazyMemmapStorage), which stores data on disk.
...
To store your buffer on the GPU, you can use [`LazyTensorStorage`](../reference/generated/torchrl.data.replay_buffers.LazyTensorStorage.html#torchrl.data.replay_buffers.LazyTensorStorage), passing the desired device.
...
[`DDPGLoss`](../reference/generated/torchrl.objectives.DDPGLoss.html#torchrl.objectives.DDPGLoss) class. This is the easiest way of utilising DDPG:
...
episode_reward_mean_adversary = 3.0, episode_reward_mean_agent = -3.0: 10%|█ | 1/10 [00:02<00:25, 2.88s/it]
episode_reward_mean_adversary = 4.0, episode_reward_mean_agent = -4.0: 20%|██ | 2/10 [00:05<00:23, 2.91s/it]
episode_reward_mean_adversary = 1.0, episode_reward_mean_agent = -1.0: 30%|███ | 3/10 [00:08<00:20, 2.92s/it]
episode_reward_mean_adversary = 110.0, episode_reward_mean_agent = -110.0: 40%|████ | 4/10 [00:11<00:17, 2.95s/it]
episode_reward_mean_adversary = 209.0, episode_reward_mean_agent = -209.0: 50%|█████ | 5/10 [00:14<00:14, 2.97s/it]
episode_reward_mean_adversary = 224.0, episode_reward_mean_agent = -224.0: 60%|██████ | 6/10 [00:17<00:11, 2.96s/it]
episode_reward_mean_adversary = 1035.0, episode_reward_mean_agent = -1035.0: 70%|███████ | 7/10 [00:19<00:08, 2.70s/it]
episode_reward_mean_adversary = 1147.0, episode_reward_mean_agent = -1147.0: 80%|████████ | 8/10 [00:22<00:05, 2.53s/it]
episode_reward_mean_adversary = 722.0, episode_reward_mean_agent = -722.0: 90%|█████████ | 9/10 [00:24<00:02, 2.43s/it]
episode_reward_mean_adversary = 867.0, episode_reward_mean_agent = -867.0: 100%|██████████| 10/10 [00:26<00:00, 2.38s/it]
```
...
![multiagent competitive ddpg](../_images/sphx_glr_multiagent_competitive_ddpg_001.png)
...
[here](../reference/envs_recorders.html#environment-recorders).
...
- How we can use [`tensordict.TensorDict`](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict) to carry multi-agent multi-group data;
...
Also do remember to check out our tutorial: [Multi-Agent Reinforcement Learning (PPO) with TorchRL Tutorial](multiagent_ppo.html).
...
[PettingZoo](https://pettingzoo.farama.org/) and VMAS contain many more scenarios.
...
**Total running time of the script:** (0 minutes 26.773 seconds)
...
[`Download Python source code: multiagent_competitive_ddpg.py`](../_downloads/d30bb6552cc07dec0f1da33382d3fa02/multiagent_competitive_ddpg.py)

[`Download zipped: multiagent_competitive_ddpg.zip`](../_downloads/5128e829b7152f5a40050c479b691f69/multiagent_competitive_ddpg.zip)
